...
import Elemento from '../Elementos/Elementos'; // importa los elemenotos del archivo elementos
import "./Lista.css"
const Lista = ({ Tareas, MarcarCompleto, Eliminar, modificar }) => { // se declaran las variables Tareas, MarcarCompleto, Eliminar, modificar
  return (
    <div className='lista'>
      ...
      {Tareas.length > 0 ? Tareas.map((Tarea) => ( //Utiliza el método map para iterar sobre la matriz de tareas (Tareas). Por cada tarea, se renderiza un componente Elemento, es decir que por cada elemento agregado se mostrara el el nombre el boton modificar, eliminar, marcar completo
        <Elemento
